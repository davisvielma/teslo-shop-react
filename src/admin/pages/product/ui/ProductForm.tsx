...
export const ProductForm = ({
  ...
  const [
    ...
    control,
    name: ["sizes", "tags", "stock", "files", "images"],
  });
  ...
  const addTag = () => {
    ...
    if (newTag.trim() && !getValues("tags").includes(newTag.trim())) {
      const tagSet = new Set<string>(getValues("tags"));
      tagSet.add(newTag);
  ...
  const addSize = (size: Size) => {
    const sizeSet = new Set<Size>(getValues("sizes"));
    sizeSet.add(size);
  ...
  const deleteImage = (image: string) => {
    ...
    const filterImages = currentImages.filter((img) => img !== image);
